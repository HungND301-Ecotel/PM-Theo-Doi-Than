...
const DEFAULT_COLORS = ['#6366f1', '#06b6d4', '#f59e0b', '#ef4444', '#8b5cf6', '#10b981']
      ...
      <CardContent>
        ...
        <ResponsiveContainer width="100%" height="100%">
          <RechartsLineChart data={data} margin={{ top: 5, right: 20, left: 0, bottom: 5 }}>
            ...
            {series.map((s, i) => {
              const color = normalizeColor(s.color || DEFAULT_COLORS[i % DEFAULT_COLORS.length])
              const dotColorVal = s.dotColor ? normalizeColor(s.dotColor) : color
              return (
                <React.Fragment key={s.dataKey}>
                  ...
                  <Line
                    ...
                    dot={showDots ? { fill: dotColorVal, r: 3.5, stroke: dotColorVal } : false}
                    activeDot={{ r: 5.5, strokeWidth: 2, stroke: 'var(--background)', fill: dotColorVal }}
                    fill={s.showArea ? `url(#gradient-${s.dataKey})` : undefined}
                  />
                </React.Fragment>
              )
            })}
